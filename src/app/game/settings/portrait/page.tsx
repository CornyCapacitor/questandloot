...
const PortraitPage = () => {
  ...
  return (
    <div className="w-full h-full flex flex-col justify-start items-center p-2 overflow-y-auto">
      {Object.entries(portraits).map(([race, portraits]) => (
        <div key={race} className="w-full">
          <h1 className="font-semibold text-white text-center text-2xl">{capitalize(race)}</h1>
          <div className="flex content-start flex-wrap gap-4 p-2">
            {portraits.map((portrait) => (
              <Image
                ...
                width={250}
                height={250}
                className="border rounded-sm hover:bg-slate-700 hover:ring hover:ring-orange-500 transition cursor-pointer"
